...
describe Playlist do
  ...
  it 'creates playlists of favorite tracks' do
    ...
    expect(last_response.header["Location"]).to match(/favorites\/\d+$/)
    playlist_id = last_response.header["Location"].gsub(/.*\//, '')
    expect(Playlist[playlist_id.to_i].auto_generated).to eq(true)
  end

  it 'creates playlists of favorite tracks' do
    get "/surprise"
    expect(last_response).to be_redirect
    expect(last_response.header["Location"]).to match(/surprise\/\d+$/)
    playlist_id = last_response.header["Location"].gsub(/.*\//, '')
    expect(Playlist[playlist_id.to_i].auto_generated).to eq(true)
  end

  it 'has autogenerated playlists from surprise' do
    get "/surprise/#{surprise.id}"
    expect(last_response).to be_ok
    expect(last_response).to match(/Bemused/)
  end

  it 'has autogenerated playlists from favorites' do
    get "/favorites/#{favorites.id}"
    expect(last_response).to be_ok
    expect(last_response).to match(/Bemused/)
  end
